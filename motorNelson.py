...
def distanciaCentral(port):
	if robot.getDistance(port,SEGUIDOR) < prom[port]:
		return 1
	else:
		return 0

def distanciaLateral(port):
	if robot.getDistance(port,SEGUIDOR) < (prom[port]-10000):
		return 1
	else:
		return 0
...
while True:
	...
	while not(frente): #frente == 0
		...
		routeDer = router(sensorGrisDer)
		frente = (frenteOrto) and (frenteDiag)
		robot.set2MotorSpeed(routeIzq , velMotorIzq, routeDer, velMotorDer,SEGUIDOR)
		print('fase 1 normal        : '+str(routeIzq)+' Izq   Der '+str(routeDer)+'  sensorDistDel '+str(frente))
		
	robot.set2MotorSpeed(1 , velMotorIzq, 1, velMotorDer,SEGUIDOR)
	time.sleep(0.4)
	print('fase 1 normal reverse: '+str(routeIzq)+' Izq   Der '+str(routeDer)+'  sensorDistDel '+str(frente))


	robot.set2MotorSpeed(1 , velMotorIzq, 0, velMotorDer,SEGUIDOR)
	time.sleep(2)

	robot.set2MotorSpeed(0 , velMotorIzq, 0, velMotorDer,SEGUIDOR)
	print('HARDCODE')
	time.sleep(1.5)
	...
	while (blanco):
		...
		if (flancoOrto):
			...
			robot.set2MotorSpeed(0, velMotorIzq, 1, velMotorDer,SEGUIDOR)
			time.sleep(0.3)	
	...
	if not(blanco):			
		...
		robot.set2MotorSpeed(1, velMotorIzq+50, 0, velMotorDer,SEGUIDOR)
